...
export const getPaperTrades = (status?: string) =>
  api.get(`/paper-trades${status ? `?status=${status}` : ''}`)
...
export const listExecutionOrders = (status?: string) =>
  api.get(`/execution/orders${status ? `?status=${status}` : ''}`)
